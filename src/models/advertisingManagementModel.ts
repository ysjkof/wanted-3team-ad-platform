...
import { CreateAdvertisingInpus, ModifyAdvertisingInpus } from '../hook/useAdvertisingManagement';
...
class AdvertisingManagementModel {
  ...
  private readonly getAdvertisingId = (arr: { id: number }[]) => arr.sort((a, b) => b.id - a.id)[0].id + 1 || 1;
...
export default new AdvertisingManagementModel(
  new HttpRequest<AdvertisingManagement>(axiosInstance, END_POINT_ADVERTISING_MANAGEMENT),
);
